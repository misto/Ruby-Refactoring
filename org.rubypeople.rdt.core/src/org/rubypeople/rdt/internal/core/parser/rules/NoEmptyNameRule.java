...
public class NoEmptyNameRule implements ParseRule {
	 ...
	 * @param token
	 */
	public NoEmptyNameRule(RubyElement token) {
		this.element = token;
	}
	...
	public boolean isAllowed() {
		if (element.isType(RubyElement.GLOBAL)) {
			return element.getName().length() > 1;
		}
		if (element.isType(RubyElement.INSTANCE_VAR)) {
			return element.getName().length() > 1;
		}
		if (element.isType(RubyElement.CLASS_VAR)) {

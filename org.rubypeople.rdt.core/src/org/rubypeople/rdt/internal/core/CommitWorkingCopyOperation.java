 ...
 * the Ruby Model up-to-date with the current contents of the working
 * copy.
 *
 ...
 * <li>force flag is <code>false</code> - in this case an <code>RubyModelException</code>
 * 	is thrown</li>
 ...
 *
 * A RubyModelOperation exception is thrown either if the commit could not
 * be performed or if the new content of the compilation unit violates some Ruby Model
...
public class CommitWorkingCopyOperation {
	...
	/**
	 * @exception RubyModelException if setting the source
...
//			setAttribute(HAS_MODIFIED_RESOURCE_ATTR, TRUE); 
	...
	public IRubyModelStatus verify() {
		...
		if (!cu.isWorkingCopy()) {
			return new RubyModelStatus(IRubyModelStatusConstants.INVALID_ELEMENT_TYPES, cu);
		}
		if (cu.hasResourceChanged() && !this.force) {
			return new RubyModelStatus(IRubyModelStatusConstants.UPDATE_CONFLICT);
		}

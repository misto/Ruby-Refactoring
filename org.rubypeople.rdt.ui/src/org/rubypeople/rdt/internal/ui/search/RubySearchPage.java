...
public class RubySearchPage extends DialogPage implements ISearchPage, ISymbolTypes {
    ...
    private ISearchQuery getSearchQuery() {
        ...
        return new RubySearchQuery(patternData.getPattern(), patternData.getSearchFor());
    }
    ...
    /*
     * private void setLimitTo(int searchFor, int limitTo) { if (!(searchFor ==
     * TYPE || searchFor == INTERFACE) && limitTo == IMPLEMENTORS) { limitTo=
     * REFERENCES; }
     * 
    ...
    private SearchPatternData getPatternData() {
        ...
        match = new SearchPatternData(searchForManager.getSelectedSymbolType(), getLimitTo(),
                pattern, fCaseSensitive.getSelection(), getContainer().getSelectedScope(),
    ...
    private void doPatternModified() {
         ...
         * fInitialData.getJavaElement(); } else {
    ...
    class SearchForManager {
        ...
        public int getSelectedSymbolType() {
            for (int i = 0; i < fSearchFor.length; i++) {
                if (fSearchFor[i].getSelection()) { return ((Integer) fSearchFor[i].getData())
                        .intValue(); }
            }
            Assert
                    .isTrue(false,
                            "Error in RubySearchPage: There should always be a selected symbol type to search for"); //$NON-NLS-1$
            return CLASS_SYMBOL;
        }

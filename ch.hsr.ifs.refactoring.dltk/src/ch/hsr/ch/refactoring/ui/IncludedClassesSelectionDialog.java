...
public class IncludedClassesSelectionDialog extends OpenTypeSelectionDialog2 implements ClassesSelectionDialog {
	...
	public IncludedClassesSelectionDialog(IDocumentProvider doc, String title, String methodname) {
		super(DLTKUIPlugin.getActiveWorkbenchShell(), false, PlatformUI.getWorkbench().getProgressService(),
				SearchEngine.createWorkspaceScope(RubyUILanguageToolkit.getInstance().getCoreToolkit()), IDLTKSearchConstants.TYPE, RubyUILanguageToolkit.getInstance());
		setMessage(title);
		this.doc = doc;
		setTitle(title);
	}
